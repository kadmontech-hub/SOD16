...
function chatMessageTemplate(message){
  ...
  return `<article class="sod-chat-message ${isUser?'user':'assistant'} ${message.status==='error'?'error':''}" data-message-id="${escapeHtml(message.id)}">
    ${isUser?'':`<div class="sod-chat-avatar" aria-hidden="true"><img src="${VISUALS.hubSodIcon}" alt=""></div>`}
    <div class="sod-chat-message-body">
      <div class="sod-chat-message-meta"><strong>${roleLabel}</strong>${message.legacy?'<span>historial local anterior</span>':''}${message.status==='sending'?'<span>enviando…</span>':''}</div>
      <div class="sod-chat-copy">${renderChatText(message.text)}</div>
      ${message.status==='error'?`<div class="sod-chat-message-error"><span>No se pudo enviar.</span><button class="btn btn-small" data-retry-message="${escapeHtml(message.id)}">Reintentar</button></div>`:''}
    </div>
  </article>`;
}

function conversationExperienceView(){return{
  title:'Hablar con SØD',
  html:`<main id="app-main" class="sod-chat-world" style="--sod-chat-bg:url('${VISUALS.talk}')" tabindex="-1">
    <div class="sod-chat-bg"></div><div class="sod-chat-shade"></div>
    <section class="sod-chat-shell">
      <aside class="sod-chat-rail" aria-label="Contexto de conversación">
        <div class="sod-chat-presence-card">
          <div class="sod-chat-core" data-chat-core><img src="${VISUALS.coreListen}" alt="Presencia visual de SØD"></div>
          <div><p class="eyebrow">SØD · PRESENCIA</p><strong data-presence-label>Presente</strong><small data-presence-copy>Escuchando sin interrumpir.</small></div>
        </div>
        <button class="btn btn-primary sod-new-conversation" data-new-conversation>Nueva conversación</button>
        <div class="sod-chat-rail-section"><p class="eyebrow">ESTADO</p><div class="sod-chat-status-row"><span class="dot"></span><div><strong>Modo local</strong><small>Esta conversación todavía no se sincroniza entre dispositivos.</small></div></div><div class="sod-chat-status-row"><span>◇</span><div><strong>Memoria longitudinal</strong><small>Pendiente del próximo milestone de backend.</small></div></div></div>
        <div class="sod-chat-rail-section" data-current-session hidden><p class="eyebrow">ESTA SESIÓN</p><button class="sod-current-session"><span>Ø</span><div><strong>Conversación actual</strong><small data-session-count>0 mensajes</small></div></button></div>
        <div class="sod-chat-rail-footer"><a href="/privacidad" data-link>Privacidad y memoria</a><a href="/hub" data-link>← Volver al Hub</a><span>Cuenta y sincronización · próxima fase</span></div>
      </aside>
      <section class="sod-chat-main">
        <header class="sod-chat-header"><button class="sod-chat-rail-toggle" data-chat-rail-toggle type="button" aria-label="Abrir contexto de conversación">Ø</button><div class="sod-chat-header-copy"><p class="eyebrow">HABLAR CON SØD</p><h1>Conversación de claridad</h1></div><div class="sod-chat-header-state"><span class="dot"></span><span>Presente</span></div></header>
        <div class="sod-chat-scroll" data-chat-scroll>
          <div class="sod-chat-stream" data-chat-stream role="log" aria-live="polite" aria-relevant="additions text"></div>
        </div>
        <div class="sod-chat-composer-zone">
          <form class="sod-chat-composer" data-chat-form>
            ...
            <button class="sod-chat-send" type="submit" aria-label="Enviar mensaje"><span>↑</span></button>
          </form>
          <div class="sod-chat-composer-meta"><span>Enter para enviar · Shift+Enter para nueva línea</span><span>No se crean Semillas ni Códigos automáticamente.</span></div>
        </div>
      </section>
    </section>
  </main>`,
  mount(){
    ...
    const sendButton=document.querySelector('.sod-chat-send');
    const core=document.querySelector('[data-chat-core] img');
    const chatWorld=document.querySelector('.sod-chat-world');
    const presenceLabel=document.querySelector('[data-presence-label]');
    const presenceCopy=document.querySelector('[data-presence-copy]');
    const sessionBlock=document.querySelector('[data-current-session]');
    ...
    const controller=createConversationController({
      api,
      sessionProvider,
      legacyMessages:store.get().dialogue||[],
      onChange:render,
    });

    function isNearBottom(){return scroll.scrollHeight-scroll.scrollTop-scroll.clientHeight<140}
    function resizeComposer(){input.style.height='auto';input.style.height=`${Math.min(input.scrollHeight,220)}px`}
    function setPresence(chatState){
      if(chatState.pending){core.src=VISUALS.coreProcess;presenceLabel.textContent='Procesando';presenceCopy.textContent='SØD está ordenando lo que trajiste.';return}
      const last=chatState.messages.at(-1);
      if(last?.role==='assistant'){core.src=VISUALS.coreResponse;presenceLabel.textContent='Presente';presenceCopy.textContent='La conversación continúa cuando vos quieras.';return}
      core.src=VISUALS.coreListen;presenceLabel.textContent='Presente';presenceCopy.textContent='Escuchando sin interrumpir.'
    }
    function emptyState(){return `<section class="sod-chat-empty"><div class="sod-chat-empty-core"><img src="${VISUALS.coreListen}" alt=""></div><p class="eyebrow">SØD · AGENTE DE CLARIDAD MENTAL</p><h2>¿Qué está ocupando tu mente en este momento?</h2><p>No necesitás formularlo perfectamente.</p><div class="sod-chat-starters"><button type="button" data-starter="Necesito ordenar una decisión que estoy postergando.">Ordenar una decisión</button><button type="button" data-starter="Hay algo que se repite y quiero entender por qué.">Observar un patrón</button><button type="button" data-starter="Tengo demasiadas ideas y no sé dónde poner mi energía.">Reducir el ruido</button></div></section>`}
    function thinkingState(){return `<article class="sod-chat-message assistant thinking"><div class="sod-chat-avatar"><img src="${VISUALS.hubSodIcon}" alt=""></div><div class="sod-chat-message-body"><div class="sod-chat-message-meta"><strong>SØD</strong><span>presente</span></div><div class="sod-thinking"><i></i><i></i><i></i></div></div></article>`}
    function render(chatState){
      ...
      setPresence(chatState);
      sessionBlock.hidden=messages.length===0;
      sessionCount.textContent=`${messages.length} ${messages.length===1?'mensaje':'mensajes'}`;
      stream.querySelectorAll('[data-starter]').forEach(button=>button.onclick=()=>{input.value=button.dataset.starter;resizeComposer();input.focus()});
    ...
    document.querySelector('[data-chat-rail-toggle]')?.addEventListener('click',()=>chatWorld.classList.toggle('show-chat-rail'));
    document.querySelector('[data-new-conversation]').onclick=()=>{
      if(!controller.getState().messages.length){controller.newConversation();input.focus();return}
      confirmAction('¿Querés iniciar una conversación nueva? La sesión local actual dejará de mostrarse en este dispositivo.',()=>{controller.newConversation();draft='';input.value='';resizeComposer();input.focus()});
    };
    controller.refreshSession().finally(()=>render(controller.getState()));
    render(controller.getState());resizeComposer();setTimeout(()=>input.focus(),80);
    return()=>{};
  }
}}
