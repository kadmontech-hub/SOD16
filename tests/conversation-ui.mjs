...
const [views, apiSource, controllerSource, styles, sw] = await Promise.all([
...
assert.match(views, /¿Qué está ocupando tu mente en este momento\?/);
assert.match(views, /No necesitás formularlo perfectamente\./);
...
assert.match(views, /No se crean Semillas ni Códigos automáticamente/);
